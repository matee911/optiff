...
Inside ImageSourceData embedded in a little-endian TIFF the descriptor is
byte-swapped as well. Verified on the `cinf` and `GenI` blocks:

- numbers (`uint32`, `uint64`, `double`) are **little-endian**
- 4-character codes (OSType, classID, length-less keys) are **reversed**
  byte by byte: `gnol` is `long`, `cjbO` is `Objc`, `sLlV` is `VlLs`
...
DESCRIPTOR_OFFSETS: dict[str, int] = {
    "cinf": 4,  # uint32 version (16)
    "GenI": 4,  # uint32 version (16)
    "CAI ": 8,  # uint32 version (3) + uint32 descriptor version (16)
}
...
@dataclass
class Descriptor:
    """A descriptor: a name, a class and named fields."""
...
class _Cursor:
    """Reader for a byte-swapped stream."""
...
def _read_value(cursor: _Cursor, ostype: str, depth: int) -> Any:
    if depth > MAX_DEPTH:
        raise DescriptorError(f"descriptor nesting exceeded {MAX_DEPTH} levels")
...
def parse(data: bytes, offset: int = 0) -> ParsedDescriptor:
    ...
    >>> stream = (
    ...     (1).to_bytes(4, "little") + b"\\x00\\x00"      # empty name
    ...     + (0).to_bytes(4, "little") + b"llun"          # classID "null"
    ...     + (1).to_bytes(4, "little")                    # one field
    ...     + (5).to_bytes(4, "little") + b"major"         # key
