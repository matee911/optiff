...
Repeats exist to separate a possible level 3/4 size inversion (documented
for real files elsewhere in this repo) from measurement noise, so a median
of several runs is reported rather than a single timing. CI does not run
this: timings from a shared runner measure the runner, not the code.

This tool prints numbers and writes an SVG; it does not itself validate or
endorse any particular deflate level as a default - that recommendation, if
made elsewhere in this repo, rests on its own, separately-stated evidence.

Usage:
    python -m tools.benchmark --out /tmp/levels.svg
"""
...
#: Default is smaller so a run finishes in a couple of minutes; pass
#: `--scale 1.0` for a run at the full reference canvas.
DEFAULT_SCALE = 0.25
